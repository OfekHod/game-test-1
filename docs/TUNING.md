...
| Fog carve set | `FOG_NEAR_MAX` | 32 | How many near trees the fog's shadow fan uses. The vision *test* always uses all of them; this caps only what is drawn. Measured at 0.2 ms a frame — this cap is not where the fog's time goes. |
| Fog surface | `FOG_SCALE` | 0.5 | The fraction of the canvas the fog mask and sheet are built at before being stretched over it. **The whole frame hangs off this**: the blur and the stretch both cost per source pixel, so 0.8 → 0.5 took an open-world frame from 33 ms to 20 and the worst frame from 54 to 32, with no visible difference (FINDINGS). 0.4 buys another 3 ms; 1.0 costs 45. `FOG_EDGE_SOFT` scales with it, so the feather on screen does not change. |
